...
public class AfinalClassUtils {
	...
	public static Object getPrimaryKeyValue(Object entity) {
		return AfinalFieldUtils.getFieldValue(entity,AfinalClassUtils.getPrimaryKeyField(entity.getClass()));
	}
	...
	public static List<Property> getPropertyList(Class<?> clazz) {
		...
		try {
			...
			for (Field f : fs) {
				//必须是基本数据类型和没有标瞬时态的字段
				if(!AfinalFieldUtils.isTransient(f)){
					if (AfinalFieldUtils.isBaseDateType(f)) {
						...
						property.setColumn(AfinalFieldUtils.getColumnByField(f));
						property.setFieldName(f.getName());
						property.setDataType(f.getType());
						property.setDefaultValue(AfinalFieldUtils.getPropertyDefaultValue(f));
						property.setSet(AfinalFieldUtils.getFieldSetMethod(clazz, f));
						property.setGet(AfinalFieldUtils.getFieldGetMethod(clazz, f));
						property.setField(f);
	...
	public static List<ManyToOne> getManyToOneList(Class<?> clazz) {
		...
		try {
			Field[] fs = clazz.getDeclaredFields();
			for (Field f : fs) {
				if (!AfinalFieldUtils.isTransient(f) && AfinalFieldUtils.isManyToOne(f)) {
					...
					mto.setColumn(AfinalFieldUtils.getColumnByField(f));
					mto.setFieldName(f.getName());
					mto.setDataType(f.getType());	
					mto.setSet(AfinalFieldUtils.getFieldSetMethod(clazz, f));
					mto.setGet(AfinalFieldUtils.getFieldGetMethod(clazz, f));
	...
	public static List<OneToMany> getOneToManyList(Class<?> clazz) {
		...
		try {
			Field[] fs = clazz.getDeclaredFields();
			for (Field f : fs) {
				if (!AfinalFieldUtils.isTransient(f) && AfinalFieldUtils.isOneToMany(f)) {
					
					OneToMany otm = new OneToMany();
					
					otm.setColumn(AfinalFieldUtils.getColumnByField(f));
					...
					otm.setSet(AfinalFieldUtils.getFieldSetMethod(clazz, f));
					otm.setGet(AfinalFieldUtils.getFieldGetMethod(clazz, f));
