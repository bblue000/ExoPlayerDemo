...
import net.tsz.afinal.db.utils.AfinalFieldUtils;
...
public class TableInfo {
	...
	public static  TableInfo get(Class<?> clazz){
		...
		if( tableInfo == null ){
			...
			tableInfo.setTableName(AfinalClassUtils.getTableName(clazz));
			tableInfo.setClassName(clazz.getName());
			
			Field idField = AfinalClassUtils.getPrimaryKeyField(clazz);
			if(idField != null){
				Id id = new Id();
				id.setColumn(AfinalFieldUtils.getColumnByField(idField));
				id.setFieldName(idField.getName());
				id.setSet(AfinalFieldUtils.getFieldSetMethod(clazz, idField));
				id.setGet(AfinalFieldUtils.getFieldGetMethod(clazz, idField));
			...
			List<Property> pList = AfinalClassUtils.getPropertyList(clazz);
			if(pList!=null){
			...
			List<ManyToOne> mList = AfinalClassUtils.getManyToOneList(clazz);
			if(mList!=null){
			...
			List<OneToMany> oList = AfinalClassUtils.getOneToManyList(clazz);
			if(oList!=null){
